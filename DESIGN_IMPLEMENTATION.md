...
## Обновление: GitHub Actions preview + фикс сеток

В этой версии дополнительно исправлены визуальные съезды кнопок и бейджей:

- `Reply opportunities` теперь имеет стабильные пять колонок, включая отдельную колонку под CTA.
- `Top posts` и `Growth Queue` получили фиксированные минимальные ширины колонок и адаптивные правила.
- Кнопки `primary/soft/ghost/link` не сжимаются и не выталкивают соседний контент.
- Верхняя панель корректно переносится на узких экранах.
- Добавлен интерактивный `Detail side panel`: клик по посту, reply opportunity или отчету открывает правую панель деталей без ухода со страницы.
- Добавлены кликабельные переключатели фильтров/guardrails и выбор шаблона Research Scan.
- Workflow переведен на GitHub Actions Pages-сценарий: сборка `frontend` → публикация `frontend/dist`.
