  ...
  return (
    <div className={`opportunity-list ${compact ? 'compact' : ''}`}>
      {opportunities.map((op) => (
        compact ? (
          <div className="opportunity-row compact" key={op.account}>
            <div className="op-main">
              <Avatar label={op.account.slice(1, 3).toUpperCase()} />
              <div>
                <strong>{op.account}</strong>
                <small>{op.reason}</small>
              </div>
            </div>
            <span className="mono-value">{op.audience}</span>
            <Badge tone={op.velocity === 'Высокая' ? 'success' : 'warning'}>{op.velocity}</Badge>
            <button
              className="soft-btn open-btn"
              type="button"
              onClick={() => onOpen?.(op.suggestion, op.reason, `${op.account} · ${op.audience} · ${op.velocity} velocity`)}
            >
              Открыть
            </button>
          </div>
          ...
          <div className="opportunity-row" key={op.account}>
            ...
            <button
              className="soft-btn suggestion-btn"
              type="button"
              onClick={() => onOpen?.(op.suggestion, op.reason, `${op.account} · ${op.audience} · ${op.velocity} velocity`)}
              title={op.suggestion}
            >
