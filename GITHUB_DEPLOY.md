...
- `frontend/vite.config.ts` — base вычисляется автоматически: в GitHub Actions берется имя репозитория, локально используется `./`.
...
Дальше на GitHub: `Settings → Pages → Build and deployment → Source → GitHub Actions`.
